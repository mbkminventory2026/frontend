...
import { PlusIcon, EyeIcon, ScissorsIcon } from 'lucide-vue-next';

import {
    getSpreadingCuttingPlans,
    type SpreadingCuttingPlanListItem,
} from '@/api/spreading-cutting-plan/spreading-cutting-plan';
...
// ─── Table State ───────────────────────────────────────
const data = ref<SpreadingCuttingPlanListItem[]>([]);
const totalCount = ref(0);
const isLoading = ref(false);

// ─── Permission ────────────────────────────────────────
const canCreate = computed(() => hasPermission('CUTTING_PLAN_CREATE'));
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await getSpreadingCuttingPlans({ page, pageSize, search: filter, sortBy, sortDesc });
        data.value = response.results;
        totalCount.value = response.count;
    } catch (error) {
        console.error('Gagal fetch Spreading & Cutting Plans:', error);
    } finally {
...
const { table, searchTerm, onSearch, clearFilter } = useTable({
    ...
    columns: [
        ...
        { header: 'Model', accessorKey: 'model' },
        {
            ...
            id: 'actions',
            cell: ({ row }) => {
                const id = row.original.id_spreading_cutting_plan;
                return h('div', { class: 'flex gap-2 justify-center items-center' }, [
                    h(Button, {
                        variant: 'outline',
                        size: 'sm',
                        class: 'shadow-xs border-neutral-300',
                        onClick: () => router.navigate({ to: '/spreading-cutting-plan/$id', params: { id: String(id) } })
                    }, () => [
    ...
    search: search,
    schema: spreadingCuttingPlanSchema,
});
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between border-b pb-5 border-neutral-100 gap-4">
            <div class="flex items-center gap-3">
                <div class="bg-neutral-50 border border-neutral-200/80 p-2.5 rounded-xl shadow-sm">
                    <ScissorsIcon class="w-6 h-6 text-neutral-600" />
                </div>
                <div>
                    <h1 class="text-2xl font-bold tracking-tight text-neutral-900">Spreading & Cutting Plans</h1>
                    <p class="text-[13px] text-neutral-500 mt-1">Daftar rencana spreading, detail roll, reject, dan rasio size pemotongan kain.</p>
                </div>
            </div>
            <div class="flex items-center gap-3" v-if="canCreate">
                <Button @click="router.navigate({ to: '/spreading-cutting-plan/create' })" variant="outline" class="shadow-sm border-neutral-300">
                    <PlusIcon class="w-4 h-4 mr-2" />
                    Tambah Spreading Plan
                </Button>
            </div>
        </div>
