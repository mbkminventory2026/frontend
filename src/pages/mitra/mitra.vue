    ...
    columns: [
        ...
        if (hasPermission('MASTER_MITRA_UPDATE')) {
            buttons.push(h(Button, {
                variant: 'ghost',
                size: 'sm',
                onClick: () => router.navigate({ to: '/mitra/edit/$id', params: { id: String(id) } })
            }, () => [
...
<template>
    ...
    >
        <template #actions>
            <Button v-if="hasPermission('MASTER_MITRA_CREATE')" @click="router.navigate({ to: '/mitra/create' })" variant="outline">
