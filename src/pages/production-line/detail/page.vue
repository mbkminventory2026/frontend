...
import { getProductionLineById } from '@/api/production-master/production-master';
...
const params = useParams({ from: '/_authenticated/production-line/$id' });
...
const fetchData = async () => {
    ...
    try {
        const response = await getProductionLineById(id.value);
...
<template>
    <div class="container mx-auto py-8 max-w-5xl">
        <div v-if="isLoading" class="flex flex-col items-center justify-center min-h-[400px] gap-4">
            ...
            <p class="text-muted-foreground animate-pulse">Memuat data production line...</p>
        </div>

        <div v-else-if="values && values.id_production_line">
            <!-- Header Section -->
            <div class="flex flex-col md:flex-row gap-6 items-center mb-8">
                ...
                <div class="flex-1 space-y-1 text-center md:text-left">
                    ...
                    <p class="text-muted-foreground">ID Line: {{ values.id_production_line }}</p>
                </div>
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    ...
                    <Button
                        v-if="hasPermission('PRODUCTION_LINE_UPDATE')"
                        @click="router.navigate({ to: '/production-line/edit/$id', params: { id } })"
                        class="flex-1 md:flex-none"
                    >
                        <PencilIcon class="w-4 h-4 mr-2" /> Edit Production Line
                    </Button>
                </div>
            </div>
                    ...
                    <Card class="overflow-hidden border-none shadow-md">
                        <CardHeader class="bg-muted/30 pb-4">
                            <CardTitle class="text-lg flex items-center gap-2">
                                ...
                                Informasi Production Line
                            </CardTitle>
                        </CardHeader>
                        <CardContent class="pt-6">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div class="space-y-1">
                                    <p class="text-xs font-medium text-muted-foreground uppercase tracking-wider">Nama Line</p>
                                    <p class="text-lg font-semibold">{{ values.name }}</p>
                                </div>
                                <div class="space-y-1">
                                    <p class="text-xs font-medium text-muted-foreground uppercase tracking-wider">ID Line</p>
                                    <p class="text-lg font-mono">{{ values.id_production_line }}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card class="bg-primary/5 border-primary/10 shadow-none">
                        ...
                        <CardContent>
                            <p class="text-xs text-primary/80 leading-relaxed">
                                Anda dapat memperbarui informasi production line ini di halaman Edit. Line ini dapat digunakan sebagai master data pada Timeline Produksi.
                            </p>
                        </CardContent>
                    </Card>
                </div>
        ...
        <div v-else class="flex flex-col items-center justify-center min-h-[400px] text-center space-y-4">
            <div class="bg-muted p-4 rounded-full">
                <Activity class="w-12 h-12 text-muted-foreground" />
            </div>
            <h2 class="text-2xl font-bold">Data Tidak Ditemukan</h2>
            <p class="text-muted-foreground">Maaf, production line dengan ID tersebut tidak dapat ditemukan atau telah dihapus.</p>
            <Button @click="router.navigate({ to: '/production-line' })">Kembali ke Daftar Production Line</Button>
        </div>
    </div>
</template>
